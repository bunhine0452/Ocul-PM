...
## v2.10.3 — 더블클릭은 창 크기만, 그리고 빈 화면의 끝

- **창 위쪽 빈 곳 더블클릭** — 타이틀바 자리인데 "더블클릭 = 새 탭"이 겹쳐 걸려 있어 **창 크기 조절과 새 탭이 동시에** 일어났습니다. 이제 크기만 조절합니다 (새 탭은 `+` · `⌘T`).
- **시작 탭 설정 → ocul-pm** — 프로젝트를 열기 전 이 화면에 들어가면 **창 전체가 하얗게 비어** 재시작밖에 길이 없었습니다. 이제 안내만 뜹니다.
- **화면 한 조각이 잘못돼도 창은 삽니다** — 오류가 그 조각에 갇히고 그 자리에 「다시 시도」가 뜹니다. 다른 탭과 탭 줄은 그대로입니다.

## v2.10.2 — 도크는 오른쪽에도, 분리 창은 끌어서 이동

- **도크 자리 셋** — 자리 바꾸기 버튼이 **아래 → 왼쪽 → 오른쪽 → 아래** 로 돕니다. 세로 두 자리는 폭을 함께 기억합니다.
- **분리한 터미널 창을 상단을 끌어 옮깁니다** — 타이틀바가 없는 창이라 잡을 자리를 앱이 내줘야 하는데 그게 빠져 있어 **창을 아예 움직일 수 없었습니다.**
- **macOS 창 분할 단축키(⌃⌥←→↑↓)** — 다른 앱에서는 되는데 여기서만 안 되던 문제. `⌘W` 를 되찾으려 메뉴를 직접 구성하면서 **어느 것이 "창" 메뉴인지 macOS 에 알려 주지 않아** 「이동 및 크기 조절」 항목이 아예 없었습니다.

## v2.10.1 — 어느 화면에서나 터미널

셸을 보려면 터미널 화면으로 들어가야 했고, 일지나 플래너를 보려면 다시 나와야 했습니다. 이제 **⌘J** 로 지금 보고 있는 화면 위에 터미널이 열립니다.

- **붙이는 자리를 고릅니다** — **아래**(가로로 넓게) 또는 **왼쪽**(세로로 길게). 경계를 끌어 크기를 조절하고, 어디에 얼마나 열어 뒀는지 기억합니다.
- **창으로 떼어냅니다** — 터미널만 담긴 창으로 분리해도 **셸은 끊기지 않고 스크롤백까지 따라옵니다**. 앱 안에는 어디로 갔는지와 되돌리는 버튼이 남습니다.
- **터미널 글자 크기를 설정에서 px 로** — 설정 → 모양에 슬라이더·px 입력칸과 **실제 터미널 글꼴 미리보기**. 터미널 화면·도크·분리한 창이 같은 값을 함께 씁니다.
- **Claude Code 의 할 일 목록이 보입니다** — 에이전트가 세우는 계획이 턴마다 뜨고, 앱 안 Claude Code 가 **작업 일지를 직접 씁니다**(ocul-pm 기록 도구 연결).
- **막히거나 다른 모델이 답하면 대화에 남습니다** — 사용 한도 초과·인증 실패·과부하·**모델 폴백**이 그동안 침묵이었습니다.
