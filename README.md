...
## 🚀 v2.12.0 — 변경 diff · 코드 맵 · 코드 검색, 셋 다 한 단계 위로

- **이미지도 diff** — 사진이 바뀌면 깨진 문자 대신 이전/현재 이미지가 나란히 (체커보드 밑판 · 크기 변화), 폰트·DB 같은 바이너리는 크기 카드로. "파일을 읽는 중…" 무한 대기 버그도 함께 제거.
- **diff 기본기** — 거터가 실제 파일 줄 번호 (조각마다 1부터 세던 가짜 번호의 끝) · 파일 옆 `+12 −4` 요약 · 한글 diff 전송 4배 부풀던 것 수정.
- **코드 맵 리디자인** — 플로팅 엣지 + 방향 화살표(A → B: A 가 B 를 사용)로 스파게티 제거, 언어 색이 스며든 노드 카드와 「허브」 태그. 검색 Enter 가 빈 화면을 만들던 버그 수정.
- **코드 검색** — 파일별 그룹 · 매치 주변만 트리밍 · 형광펜 하이라이트 · 에디터 해당 줄로 점프 · 최근 검색어 · 결과 더 보기 · 심볼 종류 필터. 느린 응답이 최신 결과를 덮어쓰던 레이스 수정.

<details>
<summary><b>지난 버전 하이라이트</b> — v2.5 부터 v2.11.0 까지</summary>
