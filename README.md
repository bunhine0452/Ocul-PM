...
- **터미널** — 앱 안 PTY 터미널. 에이전트를 여기서 돌리면 일지가 옆 화면에 쌓입니다. **⌘J 로 어느 화면에서나 도크로 띄우거나(아래·왼쪽) 별도 창으로 떼어낼 수 있고**, 셸은 그대로 이어집니다. `/plugin`·`/mcp` 처럼 CLI 자체 대화형 화면에만 사는 기능을 쓰는 탈출구이기도 합니다.
