...
## 🚀 v2.19.1 — 화면 낭독기가 Today 링을 읽습니다

- **활동 링이 낭독기에 무음이었습니다** — 이름표가 붙을 수 없는 자리에 붙어 무시됐고, 안쪽 그래픽은 숨김 처리, 숫자를 띄우는 말풍선은 마우스로만 떠서 낭독기에 남는 게 없었습니다. 이제 링 전체가 하나의 이미지로 이름을 갖고 "일지 N건 · 파일 N개 · +N/−N 줄"을 읽어 줍니다. 반대로 그 말풍선이 **마우스가 스칠 때마다** 소리 내어 알리던 것은 멈췄습니다 — 같은 숫자가 바로 아래 카드에 글자로 있습니다.
