...
## 🚀 v3.5.0 — 닿지 않던 링크와 지운 뒤의 잔해

- **눌러도 아무 일이 없던 링크들** — 터미널의 하이퍼링크와 편집기 본문의 URL 을 ⌘클릭하면 웹뷰에서 막히는 브라우저 기본 동작에 걸려 조용히 아무 일도 안 일어났습니다. 이제 밖으로 나가는 링크는 한 통로로 모여 기본 브라우저로 열립니다.
- **한글을 칠 때마다 터미널이 지웠다 다시 쓰던 것** — 음절 확정마다 지움 문자 + 재전송 왕복이 났고, 그 왕복이 진단을 정상 입력마다 울려 로그를 채웠습니다(주 4MB). 트레이스 150건 전수 집계로 글자 유실이 없음을 확인하고 확정 쌍의 삭제를 미룹니다.
- **프로젝트를 지운 뒤의 잔해** — 지운 프로젝트의 탭이 번호로 남아 누르면 실패하던 것, 삭제가 데이터베이스 재구축을 기다려 앱이 몇 초 굳던 것. 이제 행을 지우기 전에 탭·터미널 창을 닫고 정리는 뒤에서 돕니다.
- **확인 대화상자를 다시 그렸습니다** — 무엇이 종료되는지 목록으로 보여 주고, 뜬 뒤 Enter 가 곧 확인입니다. 시작 탭에서 연 설정의 네 곳이 크래시 카드이던 것, 프로젝트 밖을 가리키는 심볼릭 링크가 트리에서 평범한 파일로 보이던 것도 함께.

## v3.4.0 — 첫 기록 확인과 이어하기

- **첫 일지가 남았는지를 그 대화 단위로 확인합니다** — 일지 0건 프로젝트에 「첫 기록」 카드가 서고, 준비 → 진행 중 → 그 대화의 첫 일지 → 기록 없이 끝남 → 귀속 불명을 대화 id 로 판정합니다. 백필·옆 대화의 일지는 성공이 아니고, 기록 ≠ 검증이라고 카드가 말합니다. 첫 5분 문구 아홉 곳도 실제 조건에 맞췄습니다.
- **다음 세션이 지난 작업을 이어받습니다** — 플러그인의 SessionStart 훅이 활성 계획 뒤에 마지막 작업 일지 3건을 시작 컨텍스트에 싣고 원장 한 줄을 남깁니다. 오늘 화면의 「이어하기」 카드가 같은 자료와 「어느 대화에 포함됐는지」를 보여 줍니다(참조·도움 여부는 모른다고 적습니다).
- **일지 700건의 찾기·잇기·묶기** — `journal_search` 가 SQLite 캐시와 관련도 랭킹으로(132→25ms), 검색 화면 「일지」 스코프, 관련 후보 제안과 1클릭 잇기, 태그 사전·정리 시트, 주간 요약 층, 핫스팟·속도 카드, 미검토 대기함, 계획 로그 분리, 색인 사용량 정리.
- 숨은 프로젝트 탭으로 새던 둘 — 터미널 ⌘D 가 배경 탭까지 쪼개던 것, 새 일지 토스트의 「열기」가 다른 프로젝트 화면으로 가던 것.

## v3.3.0 — 감독관의 오탐과 닮은꼴 계획

- **앱을 두 개 띄우면 멀쩡한 감시가 끊겼다 되살아나기를 반복하던 것** — 락이 새 인스턴스로 넘어갈 때마다 감독관이 즉시 깨어나 방금 심은 프로브를 「먹통」으로 읽었습니다(2분간 52회). 인계 웨이크는 락만 놓고, 20초 안 된 프로브는 판정하지 않습니다.
- **`plan_create` 가 닮은꼴 활성 계획을 알려 줍니다** — 제목·id 가 겹치는 활성 계획이 있으면 만들지 않고 그 id·제목·hash 를 돌려줘 항목을 더하도록 권합니다(`allow_similar: true` 로 통과). 잠긴 계획은 후보가 아닙니다.
- 오늘 화면 머리글의 조사 공백(「4건 의」→「4건의」), git 저장소가 아닌 프로젝트의 브랜치 화면이 오류 카드였던 것, 검색 빈 상태의 아이콘·제목.

## v3.2.2 — 한국어 파일명과 열일곱 가지 조용한 결함

- **한국어 파일명이 git 을 지나며 8진수 이스케이프로 깨지던 것** — 백필 일지의 `files_touched`, 브랜치 화면의 파일 귀속, 일지 변경 diff 캡처(감싼 머리글을 못 알아봐 그 파일의 diff 가 빠짐)가 전부 실제 파일명으로 돌아옵니다.
- **손으로 고친 `config.toml` 에 키가 빠지면 프로젝트가 통째로 무효가 되던 것** — 빠진 값은 새 프로젝트 기본값으로 채워집니다.
- 비밀 마스킹이 겹치는 매치에서 꼬리를 남기던 것, 비ASCII 세션 id 에 색인이 패닉하던 것, 설정 화면이 열 때마다 config 를 되쓰던 것, 팔레트·계획 화면의 늦은 응답 덮어쓰기, 메뉴바 팝오버의 부분 실패·정렬, 탭 × 재진입, `[object Object]` 로그 등 열일곱 건.

## v3.2.1 — 메뉴바 클릭과 지운 폴더

- **macOS 27 에서 메뉴바 아이콘을 누르면 팝오버 대신 「열기 / 종료」 메뉴가 뜨던 것** — 이 OS 는 상태 아이템에 메뉴가 붙어 있으면 왼쪽 클릭까지 가로챕니다. 메뉴를 오른쪽 클릭 순간에만 붙였다 떼도록 바꿔, 왼쪽 클릭은 팝오버·오른쪽 클릭은 메뉴입니다.
- **앱을 켜 둔 채 Finder 에서 프로젝트 폴더를 지우면 빈 폴더가 되살아나던 것** — 삭제가 만든 파일 이벤트가 세션을 열고 그 기록이 `.oculpm/index/` 를 다시 만들었습니다. 루트가 사라지면 워처는 이벤트를 버리고 색인은 아무것도 다시 만들지 않습니다.

## v3.2.0 — 확인은 내용에 묶입니다

- **일지의 「확인」이 스위치가 아니라 약속이 됩니다** — 확인 표시는 확인한 그 본문의 지문과 함께 남고, 그 뒤 에이전트가 본문을 고치면 원장과 열람에 **「확인 뒤 내용이 변경됐어요 · 다시 검토」** 가 섭니다. 「검증됨만」 필터와 변경 검토 그룹은 그 일지를 확인된 것으로 세지 않습니다. 이전에 확인해 둔 일지는 그대로 인정합니다.
- **두 프로세스가 같은 일지를 덮던 것을 막았습니다** — 같은 분·종류·제목으로 앱과 MCP 서버(또는 병렬 세션 둘)가 동시에 쓰면 뒤가 앞을 조용히 교체할 수 있었습니다(8프로세스 재현에서 24건 중 3건 생존). 새 일지는 배타적으로 생성됩니다. 느린 프로세스가 남의 락을 지우던 것도 nonce 소유로 닫았습니다.
- **진단 탭 「워처 계측」** — 이벤트·버림·큐 깊이·처리 시간. 첫 실측: 브랜치 전환 뒤 3.6초 정착 중 실제 처리는 32ms.
- **지원 에이전트를 네 등급으로 정직하게** — 아래 「지원 에이전트」 표. 등급은 코드에서 읽었고 검증 열은 실측 기록이 있는 것만.
- **배포는 검증을 통과한 것만 공개** — 태그 커밋의 CI 통과 확인 → draft 빌드 → 서명·공증·업데이터 서명 검사 → 공개. 3.1.1 이 의존성 검사가 붉은 커밋에서 나갔던 실수를 기계가 막습니다.
- 안쪽: 800줄 넘는 파일 상위 7개를 책임별로 분할(37 → 26 파일), AI 패널 컨텍스트 왕복 4 → 2, 확장 단위 테스트 CI 편입.
