...
- **코드 트리에 점 파일** — `.oculpm/` · `.github/` · `.gitignore` 가 목록에 아예 없어 정작 가장 자주 여는 것들을 코드 화면에서 못 열었습니다. 이제 보입니다 (저장소가 무시하도록 정한 `node_modules` 같은 것은 계속 숨깁니다).
...
- **코드** — 앱 안 코드 에디터. 검색·코드맵에서 해당 줄로 바로 점프해 열고, **자동완성·진단 밑줄·호버 타입·정의로 이동·이름 바꾸기·코드 액션**(LSP)과 함께 고쳐 ⌘S 로 저장합니다. 점으로 시작하는 파일도 트리에 보입니다. 에이전트가 같은 파일을 고치면 충돌 배너가 지켜 주고, 저장 안 한 편집은 화면을 옮겨도 남습니다. 무거운 편집은 「외부 에디터로 열기」가 커서 줄까지 넘겨 줍니다.
