...
describe("화자는 한 명이다 — 해요체", () => {
  /**
   * 2026-09-10 {#copy-voice}: 사용자가 **해요체**로 정했다. 그전까지 사전은
   * 합쇼체 431 : 해요체 315 로 갈려 있었고, 무작위가 아니라 **화면별로**
   * 나뉘어 있었다 — 설정·코드·스킬은 합쇼체, Today·논의·플래너·그래프는
   * 해요체. 오래된 표면과 새 표면의 경계였다.
   *
   * 이 자리에는 예전에 "한 문자열 안에서 섞이지 않는다" 만 보는 약한 게이트가
   * 있었다. 그건 전체 말투가 갈려 있던 동안의 타협이었고, 두 가지로 고장나
   * 있었다: `돼요` 가 두 목록에 다 들어 있어 그 낱말을 쓴 문자열은 무엇이든
   * 걸렸고(해요체로 옮기자 바로 드러났다), 정작 **합쇼체 자체**는 못 막았다.
   *
   * 이제는 전부를 본다. 섞임은 한쪽이 없으면 생길 수 없으므로 이 규칙이
   * 옛 규칙을 포함한다.
   */
  const HAPSYO = /(습니다|합니다|입니다|십시오|니다)(?=[\s.,)\]"—·…]|\\n|$)/;

  it("사전에 합쇼체가 남아 있지 않다", () => {
    const hits: string[] = [];
    for (const line of bare.split("\n")) {
      const m = /"([\w.-]+)":\s*"((?:[^"\\]|\\.)*)"/.exec(line);
      if (!m) continue;
      if (HAPSYO.test(m[2])) hits.push(`${m[1]} — ${m[2].slice(0, 70)}`);
    }
    expect(hits, `합쇼체 ${hits.length}건\n${hits.slice(0, 20).join("\n")}`).toEqual([]);
  });
});
...
describe("에러는 로그가 아니라 메시지다", () => {
   ...
   * 먼저** 말하고 원문을 뒤로 민다 — "데이터베이스를 압축하지 못했어요 — {error}".
   */
  it("「… 실패: {error}」 꼴이 없다", () => {
    const hits: string[] = [];
    for (const line of bare.split("\n")) {
