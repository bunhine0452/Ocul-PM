 ...
 * (뼈대는 `lib/createStore.createIntentSlot` — Phase 4 에서 합쳤다.)
 */
...
const slot = createIntentSlot<ManualEntrySeed>("oculpm:request-manual-entry");
...
export function onManualEntryRequest(fn: (seed: ManualEntrySeed) => void): () => void {
  return slot.subscribe((seed) => fn(seed ?? {}));
}
