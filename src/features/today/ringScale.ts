...
//
// ## `k` 를 어떻게 정했나 — `lines` (2026-09-07 실측)
//
...
//
// ## `k` 를 어떻게 정했나 — `files` (2026-09-07 실측, `{#files-ring-scale}`)
//
// 같은 방법. 자료는 캐시의 `COUNT(DISTINCT file_path)` 를 워크데이별로 — 링이
// 세는 바로 그 수다(`{#distinct-files-backend}` 이후 백엔드가 답한다). 44
// 워크데이: **중앙값 55개**, 범위 2~209, p75 100, p90 170. 8월 이후 26일만
// 보면 중앙값 81, p75 129.
//
// `files` 는 `R_MID` 라 상한이 0.9312 이고, 포화 곡선에서 상한에 닿는 값은
// `k × 13.54` 다. 그래서
//
//  - `k=8`(옛값) → 상한 **108개**. 26일 중 **8일**(44일 중 9일)이 눌린다.
//    중앙값도 0.91 이라 링이 거의 늘 꽉 차 보인다 — 정보를 안 나른다.
//  - `k=20`(새값) → 상한 **271개**. 실측 44일 중 **0일**이 눌린다. 중앙값
//    0.73(8월 이후 0.80 — `lines` 의 0.79 와 같은 읽기), 가장 바쁜 하루
//    209개도 0.91 로 **상한 바로 아래**다.
//
// 상한을 없앤 게 아니라 이 저장소 위로 올린 것이다: 하루 271개를 넘기는
// 사용자에게는 여전히 걸리고, 그때 화면이 말한다.
//
// 그래도 상한은 남는다 — 이 저장소보다 더 바쁜 날은 있을 수 있다. 그래서
// 눈금을 고치는 것과 **별개로** 상한에 붙었다는 사실 자체를 값으로 내보낸다
 ...
 *
 * 2026-09-07 에 `lines` 400 → 4000, 이어서 `files` 8 → 20(둘 다 실측 분포에서
 * 골랐다 — 위 두 문단이 그 숫자다). `journals` 는 그대로다: 하루 일지 수는
 * `k=4` 로 상한이 64건이라 실측 최댓날(46건)도 안 눌린다.
 */
export const RING_K = {
  journals: 4,
  files: 20,
