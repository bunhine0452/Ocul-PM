        ...
        <div className="page flex max-w-3xl flex-col gap-5">
          ...
          <section className="rounded-lg border border-border/60 bg-card p-5">
            <div className="mb-2 flex items-center gap-2">
              ...
              {status?.installed ? (
                <span className="rounded bg-(--ok-soft) px-1.5 py-0.5 text-fs-2 font-medium text-(--ok-text)">
                  {t("plugin.installed")}
                </span>
              ) : status ? (
                <span className="rounded bg-muted px-1.5 py-0.5 text-fs-2 font-medium text-muted-foreground">
                  {t("plugin.notInstalled")}
                </span>
              ) : null}
            </div>
            <p className="text-fs-4 leading-relaxed text-muted-foreground">
              {t("plugin.blurb")}
              {status?.installed
            ...
            {!status?.installed && (
              <div className="mt-3 flex flex-col gap-1.5">
                {PLUGIN_INSTALL_COMMANDS.map((c) => (
                  ...
                  >
                    <code className="flex-1 truncate font-mono text-fs-3">{c}</code>
                    <CopyChip text={c} />
                  </div>
                ))}
              </div>
          ...
          <section className="rounded-lg border border-border/60 bg-card p-5">
            ...
            <div className="flex flex-wrap items-center gap-1.5 font-mono text-fs-3">
              ...
              <span className="text-fs-2 text-muted-foreground">{t("plugin.flowRepeat")}</span>
            </div>
            <p className="mt-2 text-fs-3 leading-relaxed text-muted-foreground">
              {t("plugin.flowNote")}
            </p>
          </section>
              ...
              {PLUGIN_COMMANDS.map((c) => (
                <div key={c.slug} className="rounded-md border border-border/60 bg-background/60 p-3">
                  <div className="flex items-center gap-2">
                    <code className="font-mono text-fs-4 font-semibold text-primary">{c.cmd}</code>
          ...
          <section className="rounded-lg border border-border/60 bg-card p-5">
            ...
            <p className="mb-3 text-fs-3 leading-relaxed text-muted-foreground">
            ...
            <div className="flex flex-col gap-1">
              {PLUGIN_TOOLS.map((tool) => (
                <div key={tool.name} className="flex items-baseline gap-2 text-[12.5px]">
                  <code className="w-32 shrink-0 font-mono text-fs-3 text-primary">
                    {tool.name}
                  </code>
