...
// PR-ACP14 — 상단바의 세션 탭 줄.
//
// 여기는 원래 "Claude Code / Claude Code 를 앱 안에서 구동" 이라는 제목+설명이
// 있던 자리다. PR-ACP14 는 둘 다 지웠고(설명은 처음 한 번 읽고 나면 영영
// 쓸모가 없는데 창에서 제일 좋은 자리를 차지한다), {#unify-chat}(2026-09-11)
// 이 **이름만** 되돌렸다 — 사이드바를 접으면 화면 이름을 말하는 곳이 없고,
// 16화면 중 이 둘만 제목이 없는 것이 "여러 손" 신호였다. 이름은 `AcpToolbar`
// 가 앞에 세운다.
//
// 이 줄은 **지금 열어 둔 대화들**이다. 상단바에서 알고 싶은 것은 "이 화면이
// 무엇인가"에 더해 "나는 어느 대화에 있는가"이고, 그 답은 탭 줄이 전환과 한
// 몸으로 준다.
//
