...
// 진입 때 뜬금없이 열려 있다. (`createSignal` — Phase 4.)
import { createSignal } from "@/lib/createStore";

const signal = createSignal();

/** 위젯을 열어 달라고 알린다 (듣는 이가 없으면 조용히 사라진다). */
export const requestUsagePanel = signal.emit;

/** 구독. 반환값을 부르면 해지된다 (effect cleanup 에 그대로 쓴다). */
export const onUsagePanel = signal.on;
