...
function CodeSample({ label }: { label: string }) {
  return (
    <pre
      role="img"
      aria-label={label}
      className="m-0 px-2.5 py-2 rounded-lg border border-border text-fs-2 leading-relaxed font-mono overflow-x-auto"
...
export function ThemeEditor({
  ...
  return (
    <div className="space-y-4">
      <p className="text-fs-2 text-muted-foreground">{t("theme.editor.hint")}</p>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <label className="space-y-1.5">
          <span className="block text-fs-2 uppercase text-muted-foreground tracking-wider">
            {t("theme.editor.name")}
          </span>
          <input
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-fs-2 text-muted-foreground/80">{t("theme.editor.familyHint")}</p>
        </div>
      </div>
      ...
      {TOKEN_GROUPS.map((group) => (
        <div key={group.id} className="space-y-2">
          <div className="flex items-center gap-1.5 text-fs-2 uppercase text-muted-foreground tracking-wider">
            <Palette size={13} />
            {t(group.titleKey)}
          </div>
              ...
              return (
                <li key={token} className="flex items-center gap-2">
                  <span className="w-40 flex-none min-w-0">
                    {labelKey && (
                      <span className="block text-fs-2 text-foreground truncate">
                        {t(labelKey)}
                      </span>
                    )}
                    <code
                      className={`block font-mono text-muted-foreground truncate ${
                        labelKey ? "text-fs-1" : "text-fs-2"
                      }`}
                    >
                      {token}
                    </code>
