      ...
      {runs.map((run) => (
        <li key={run.id} className="rounded-md border border-border/60 px-3 py-2">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-fs-2 text-muted-foreground/70 font-mono">
              {run.automation_id}
            </span>
            {run.journal_path && (
              <button
                className="text-fs-2 text-primary hover:underline cursor-pointer ml-auto"
          ...
          {run.note && (
            <p className="text-fs-2 text-muted-foreground mt-1 whitespace-pre-wrap">
              {run.note}
            </p>
          )}
        </li>
