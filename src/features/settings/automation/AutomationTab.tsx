    ...
    <>
      ...
      {!coreModel && (
        <div className="rounded-md border border-border bg-accent/20 px-3 py-2 mb-4" role="status">
          ...
          <button
            className="text-fs-2 text-primary hover:underline cursor-pointer mt-1"
      ...
      <Section title={t("automation.list.title")} description={t("automation.list.desc")}>
          ...
          {items.map((s) => {
            ...
            return (
              <li key={`${s.def.kind}:${s.def.id}`} className="card card-pad">
                ...
                <p className="text-fs-2 text-muted-foreground mt-1">
                ...
                {s.spec_error && (
                  <p className="text-fs-2 text-destructive mt-1">
                    {tError({ code: s.spec_error, detail: null })}
                  </p>
                )}
                {s.warnings.map((w) => (
                  <p key={w} className="text-fs-2 text-muted-foreground/80 mt-1">
                    {w}
                  </p>
                ))}
        ...
        {seeds.length > 0 && (
          <div className="pt-2">
            <p className="text-fs-2 text-muted-foreground mb-2">{t("automation.seeds.title")}</p>
