      ...
      <ul className="flex flex-col gap-1.5">
        {entries.map((e) => (
          <li key={e.relative_path} className="flex items-baseline gap-2 text-sm">
            <span className="shrink-0 rounded bg-muted px-1.5 py-0.5 text-fs-2 font-medium">
            ...
            <ul className="flex flex-col gap-1.5">
              {d.rows.map((c) => (
                <li key={c.sha} className="flex items-baseline gap-2 text-sm">
                  ...
                  {c.journal_count > 0 ? (
                    <span className="shrink-0 rounded bg-(--ok-soft) px-1.5 py-0.5 text-fs-2 font-medium text-(--ok-text)">
      ...
      <ul className="flex flex-col gap-1.5">
        {rows.map((f) => (
          <li key={f.path} className="flex items-center gap-2 text-sm">
            ...
            {!f.recorded ? (
              <span className="shrink-0 rounded bg-muted px-1.5 py-0.5 text-fs-2 font-medium text-muted-foreground">
