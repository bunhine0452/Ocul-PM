...
// lifecycle (listen pty-data/exit → startPtySession → onData → write), just
// rendered inside the ui_v2 .term-screen shell.
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      try {
        fitRef.current?.fit();
        if (termRef.current) {
          void commands.resizePty(sessionId, termRef.current.rows, termRef.current.cols);
        }
      } catch {
        /* fit before paint — ignore */
      }
    }, 50);
