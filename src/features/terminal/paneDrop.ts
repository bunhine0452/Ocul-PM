...
// ── 드래그 한 번의 진행 상태와 기하 (2026-09-04 이사) ─────────────────────
//
// `TerminalSurface` 에서 옮겨 왔다. 여기 있는 `Box`·`PaneBox`·`DropEdge` 만
// 참조하는 타입들이라 원래 이 파일의 식구였고, 화면 파일이 한계를 넘은 것이
// 옮길 자리를 다시 보게 만들었다.
...
export const toBox = (r: DOMRect): Box => ({
  left: r.left,
  top: r.top,
  width: r.width,
  height: r.height,
