...
export function AgentTag({ agentId }: { agentId: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 text-fs-2 mob-text-2">
        ...
        <li key={e.relative_path}>
          <button onClick={() => onOpen(e)} className="mob-card w-full text-left px-3.5 py-2.5">
            ...
            <div className="flex items-center gap-2.5 mt-1.5">
              ...
              <span className="text-fs-2 mob-text-3">{e.created_at.slice(11, 16)}</span>
              {e.files_count > 0 ? (
                <span className="text-fs-2 mob-text-3">{e.files_count} files</span>
              ) : null}
            </div>
          </button>
        </li>
