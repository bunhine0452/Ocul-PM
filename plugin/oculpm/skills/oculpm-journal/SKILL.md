...
학습 품질 (growth-log 규율): 일지는 성과 나열이 아니라 **다음 세션이 헛수고를
안 하게 하는 기록**이다 — 실패한 접근·막다른 길·의외였던 사실을 성공보다 우선
기록한다 ("X 를 시도했지만 Y 라서 안 됨"이 "X 완료"보다 값지다). 같은 실패를
반복하지 않도록 `## 메모` 에 다음 독자를 위한 학습 1줄을 남긴다.
