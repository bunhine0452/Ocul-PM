...
- **⌘K — select the code, say what to change.** Select, press ⌘K, type "make this async", and it is rewritten in place. Changes land **applied first, reviewed after**: toggle each hunk to choose what stays (turn them all off and you have the original back), and the edit is recorded in the work journal and local history as **written by an agent**.
- **The Retro and Docs screens are gone.** Retro had a large surface and little use; Docs was a permanently empty screen in projects without a `docs/` folder. Your existing `.oculpm/retro/*.md` files are **not deleted** — they are yours, and journal search still reads them.
- **Removed the seams between screens.** Roughly 1,900 half-step drifts in type size, line height, spacing, corner radius and weight were folded onto one ramp; the three ways of opening settings became one; "waiting to fill" and "genuinely empty" no longer look pixel-identical; and 700 strings were moved to a single voice. Buttons that cannot be pressed now say **why**.
- **Fixes** — a discussion doc open in two places silently overwrote the earlier edit · the phone pairing screen kept polling after it closed · deleted rules kept being billed to the context budget for 30 days · the Korean IME Enter check existed in four different versions across screens.
...
- **A new Branch screen** — the journals, plan items, commits and changed files of a three-day branch as one coordinate, with a **recorded rate** and the changes that still have no entry. Exportable as a single markdown page. Nothing new is stored; it reads the git you already have.
- **The first five minutes tell the truth.** The wizard no longer claims to have created what it hasn't (the record folder and rules file appear when you *open* a project), and Today no longer promises automation that isn't switched on. A missing Claude Code plugin is finally surfaced, and the nine screens that are empty on day one say **what to do to fill them**. The honesty panel now lets you record an entry right there — and the card whose only option used to be a paid model call now puts the free path first.
- **The same code no longer gets different colours on different screens.** Syntax highlighting is one palette and follows all five themes, the editor included. **Settings search** arrives (12 tabs, 100+ entries), and type sizes and spacing converge onto one scale — the whole app gets subtly denser.
...
- **What leaves and what never does** — [oculpm.com/privacy](https://oculpm.com/privacy). The app opens exactly **five** outbound connections (LLM requests · update checks · GitHub fetches · a one-time embedding-model download · Notion, opt-in — its OAuth exchange is brokered by oculpm.com, and your document content goes straight to Notion). Usage statistics and crash reports are **not collected at all**.
