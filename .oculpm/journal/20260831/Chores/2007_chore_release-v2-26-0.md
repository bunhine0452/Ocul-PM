...
**`docs/RELEASE.md` 가 개인 메모보다 새롭다.** 메모에는 "버전 3파일 · 랜딩 5곳" 으로
적혀 있었는데 실제로는 **5파일 · 6곳**이다(plugin.json·marketplace.json 추가, 다운로드
버튼이 2곳). 메모를 그대로 믿었으면 두 파일과 버튼 하나를 놓쳤을 것이다 — 저장소
문서가 정본이다.
