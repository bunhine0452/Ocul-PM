...
- **`ShellV2.tsx`** — owns the sidebar + a router over `state.uiV2View` across 8 screens (Today, Journal, Diff, Planner, Search, Terminal, AI panel, Code Map/Graph). **Each screen renders its own `<Toolbar>`.** `GraphScreenV2` is lazy (pulls in React Flow + dagre).
- **`features/`** — one folder per screen/domain (`today`, `oculpm` = journal, `diff`, `planner`, `search`, `terminal`, `chat` = AI panel, `graph`, `onboarding`, `settings`, `overview`, `code`, `shell`).
