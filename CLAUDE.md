...
- **`ShellV2.tsx`** — owns the sidebar + a router over `state.uiV2View` across **15 screens** (Today, Journal, Diff, Planner, Search, Code Map/Graph, Terminal, Code, Claude Code, Codex, Sessions, AI panel, Skills, Discussion, Branch — the Retro and Docs screens were removed 2026-09-08; `settings` also renders as a screen). **Each screen renders its own `<Toolbar>`.** Sidebar/⌘K palette/⌘-number shortcuts all derive from `src/lib/navRegistry.ts` (single source — the first 10 entries own ⌘1~⌘0, so append new screens at the END; the sidebar shows 13 rows because Claude Code/Codex/Sessions fold into one 「에이전트」 row). Today/Journal/Diff/Planner are eager; the rest are lazy chunks. ⌘\ jumps to the AI panel (the ⌘\ overlay chat stack was retired 2026-07-16); ⌘[ / ⌘] walk the screen history (`hooks/useNavHistory.ts`). **Three** chat surfaces, not one: `AiPanelScreenV2` (provider chat), `ClaudeCodeScreenV2` and `CodexScreenV2` (ACP).
- **`features/`** — one folder per screen/domain (`today`, `oculpm` = journal, `discussion`, `planner`, `diff`, `branch`, `search`, `graph`, `terminal`, `code`, `chat` = all three AI surfaces, `skills`, `sessions`, `onboarding`, `settings`, `projects`, `deeplink`, `theme`, `tray`, `shell`).
