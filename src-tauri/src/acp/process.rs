...
use super::session_book::SessionBook;
...
pub struct AcpState {
    ...
    /// 이 프로세스가 연 세그먼트 등록부 ({#acp-segment-close}). 어댑터가 죽거나
    /// 앱이 내려갈 때 **우리가 연 마커만** 골라 닫는다 — 자세한 이유는
    /// [`segments`](super::segments) 모듈 문서.
    pub(crate) segments: super::segments::OpenSegments,
}
...
impl AcpState {
    ...
    pub fn stop_all_blocking(&self) {
        ...
        // 열어 둔 세그먼트를 닫는다 ({#acp-segment-close}) — 안 닫으면 죽은
        // 대화의 생존 흔적이 6시간 동안 옆 대화의 게이트를 침묵시킨다.
        super::journal_gate::close_open_segments(self.segments.take_all());
...
pub async fn start(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        // 이 어댑터가 열어 둔 세그먼트도 여기서 닫는다 ({#acp-segment-close}).
        // **이 대상 것만** 꺼낸다 — 옆 어댑터의 대화는 계속 돌고 있다.
        //
        // 닫기는 `git status` 를 부르므로 블로킹이다 — `commands::acp_gate` 와
        // 같은 이유로 워커에서 재우지 않는다. 어댑터 하나가 죽는 순간 옆 대화의
        // 스트리밍이 수십 ms 멈추면, 고치려던 것보다 눈에 띄는 고장이 된다.
        let open = state.segments.take_for_target(target_id);
        let _ = tokio::task::spawn_blocking(move || super::journal_gate::close_open_segments(open))
            .await;
