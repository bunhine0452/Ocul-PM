...
//! 묶는 자리는 앱의 「세션」 화면이다 (2026-09-04 에 Today 카드에서 나왔다 — D8).
...
pub fn refusal(root: &Path, from: &str, to: &str, now: DateTime<Utc>) -> String {
    match group_of(root, from, now) {
        ...
        None => "이 세션은 아직 어느 그룹에도 묶이지 않았습니다 — 앱의 「세션」 화면에서 함께 일할 세션과 묶어 주세요 (진행 중인 태스크를 함께 하고 있다면 그 상대와는 묶지 않아도 말할 수 있습니다)".to_string(),
    }
}
