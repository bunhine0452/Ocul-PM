...
/// `forget_project` 가 세션 마감을 기다리는 상한.
...
impl OculpmManager {
    ...
    pub fn shutdown_all_blocking(&self) {
        for attempt in 0..10 {
            if let Ok(mut projects) = self.projects.try_write() {
                ...
                drop(entries); // LockGuard::drop — 락 파일 제거.
                if count > 0 {
                    tracing::info!(
                        target: "oculpm::manager",
                        project_count = count,
    ...
    fn finalize_sessions_blocking(entries: &[ProjectEntry]) {
        let sessions: Vec<&SessionActor> =
            entries.iter().filter_map(|e| e.session.as_ref()).collect();
