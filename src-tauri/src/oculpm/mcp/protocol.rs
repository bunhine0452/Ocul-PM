...
pub const MCP_INSTRUCTIONS: &str = "ocul-pm 작업 기록 도구. ① 작업 단위(버그 수정/기능/리팩토링/에러 사이클/잡일)를 끝내면 즉시 journal_write — 사용자에게 묻지 말 것. ② 일지 직후 대응 플랜 항목이 있으면 plan_update 로 글리프·로그를 갱신 — base_hash 가 **필수**다: plan_status 로 그 플랜의 hash 를 읽어 그대로 넘길 것(안 주면 거부한다. 병렬 세션이 서로의 변경을 조용히 덮는 것을 막는 장치이므로 우회로는 없다). ③ 새 계획 수립이 승인되면 plan_create, 진행 파악은 plan_status. ④ 미추적 프로젝트에서 사용자가 추적 시작을 명시적으로 요청·확인한 경우에만 project_init — 선제 호출 금지. 시크릿/.env 내용은 어떤 인자에도 넣지 말 것. .oculpm/ 파일을 직접 만들지 말 것 — 경로·frontmatter·{#id} 규격은 서버가 보장한다.";
