...
pub fn tool_definitions() -> Value {
    json!([
        ...
        {
            "name": "plan_status",
            "description": "이 프로젝트의 활성 플랜(.oculpm/planner)과 항목별 진행 상태를 반환한다. 작업 시작 전 현재 계획·다음 할 일을 파악할 때 호출.",
...
fn plan_status(root: &Path) -> Result<Value, String> {
    ...
    for f in entries.flatten() {
        ...
        plans.push(json!({
            ...
            "items": parsed.items.iter().map(|i| json!({
                "id": i.item_id,
...
mod tests {
    ...
    fn plan_status_lists_active_items() {
        ...
        assert_eq!(plans[0]["items"][1]["status"], "in_progress");
    }
