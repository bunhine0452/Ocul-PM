...
mod tests {
    ...
    fn master_templates_stay_lean_and_in_parity() {
        ...
        assert!(en <= 6_400, "en 마스터 {en} chars — 상한 6,400 (6,320 에서 올렸다: §0 의 롤업 한 줄 77자, journal-scale-round {{#rollup-first}}. 주간 요약 층을 만들어 놓고 규칙이 그것을 가리키지 않으면 에이전트는 영영 안 읽는다 — 같은 테스트가 아래에서 '읽기 도구는 있다로 부족하고 **언제 부르는지** 가 있어야 실제로 불린다'고 못박은 그 실패 양식이다. ko 는 상한 안이라 손대지 않았다. 다음 인상도 같은 급의 근거를 요구할 것)");
        assert_eq!(
            template_version(MASTER_KO),
