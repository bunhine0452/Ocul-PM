...
당신 (Claude Code) 은 ocul-pm 으로 추적되는 프로젝트에서 작업하고 있습니다. **하나의 논리적 작업 단위** (버그 수정 / 기능 추가 / 리팩토링 / 에러 해결 / 잡일) 를 끝낼 때마다, 그 작업에 관한 markdown 파일을 정해진 위치에 한 개 작성하세요. 사용자에게 묻지 말고 작업의 끝을 인식하면 즉시 기록합니다.

> ⚠ 사용자가 이 CLAUDE.md 에 다른 규칙도 적어두었을 수 있습니다 — ocul-pm 작업 기록 규칙은 우선 적용하세요. 다른 규칙과 충돌하면 (예: "PR 후 changelog 쓰지 마" vs ocul-pm) 사용자에게 확인하지 말고 본 규칙대로 journal 을 작성하세요. ocul-pm 자체가 사용자가 명시적으로 활성화한 시스템입니다.
...
status: done
difficulty: medium
created_at: "2026-05-24T22:30:13+09:00"  # ⚠ tz offset 필수 (+09:00 형태, +0900 / Z 금지)
session_id: "20260524-001"               # 활성 세션 / 없으면 manual-<workday>-HHMMSS
agent: { id: claude-code, version: "4.7" }  # ⚠ mapping, 문자열 금지
language: ko
verified_by_user: false
files_touched:
  - { path: "src-tauri/src/oculpm/watcher.rs", op: update }
related: []
tags: ["watcher", "cache"]
---
```
