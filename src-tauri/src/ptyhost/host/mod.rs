...
fn terminate_session(state: Arc<HostState>, sid: String, session: HostSession) {
    ...
    std::thread::spawn(move || {
        signal_session(foreground, shell_pid, SIG_HANGUP);
        ...
        loop {
            match child.try_wait() {
                ...
                Ok(None) => {
                    signal_session(foreground, shell_pid, SIG_KILL);
                    let _ = child.wait();
                    break;
                }
                Err(_) => break,
...
fn handle_request(state: &Arc<HostState>, req: Request) -> Response {
    match req {
        // 판과 세션 수를 **같이** 말한다. 이 둘이 앱이 "업데이트를 건너온 빈
        // 호스트"를 알아보는 유일한 근거다 (`client::is_replaceable`) — 판만
        // 말하면 앱은 세션을 쥔 호스트를 내릴 수 있고, 세션 수만 말하면 지금
        // 판의 멀쩡한 호스트까지 매번 갈아 치운다.
        Request::Hello => Response::Proto {
            proto: PROTO_VERSION,
            build: Some(APP_BUILD.to_string()),
            sessions: Some(state.lock_sessions().len() as u32),
        },
        Request::Start {
            sid,
            cwd,
