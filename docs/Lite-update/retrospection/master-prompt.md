...
- **Phase D**: 🚧 PR10 Part 1 완료 / PR10 Part 2~3 미진입 / ✅ PR11 완료 (자동 측정) / ☐ PR12 미진입
- **추가**: ✅ PR-0c 완료 (clippy 48 errors → 0). `cargo clippy --all-targets -- -D warnings` 가 이제부터 진짜 lock.

진행 중 작업: *없음*. **Phase D 거의 종료**. PR12 (빌드/서명/릴리스) 만 남음 — Apple Developer ID 등 외부 dep 가능성 → 사용자 입력 필요. PR10 Part 2 (axe-core) 도 외부 dep → 사용자 confirm 필요. PR10 Part 3 (카피 통일) + PR6.5 (LocalDiffView UX polish) 은 1.1 안전 후보.
