...
- **Phase C**: 🚧 PR6 (backend 만, 6.1 연기 / 6.2 완료 / 6.3~6.5 미진입) / ✅ PR7 (Part 1 + Part 2 모두 완료) / ✅ PR8 (Part 1~3 **모두 완료**) / ☐ PR9 미진입
- **Phase D**: ☐ PR10~PR12 미진입
- **추가**: ✅ PR-0c 완료 (clippy 48 errors → 0). `cargo clippy --all-targets -- -D warnings` 가 이제부터 진짜 lock.

진행 중 작업: *없음*. 다음 작업 후보: **PR9 (AI overlay ⌘\ + 분리 윈도우 ⌘⇧\)** — Phase C 의 마지막 큰 frontend, 또는 **PR6.3~6.5 (LocalDiffView UI)** — SidePanel 에 dock 자연스러움. Phase D (PR10~PR12) 진입 전 PR9 + PR6 의 어느 한쪽이 1.0 의 *기능 표면* 으로 필요.
