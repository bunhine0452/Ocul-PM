...
### 5.1 현재 위치 (2026-05-31)

- **Phase A**: ✅ PR0 완료 / ✅ PR1 완료 (no-op + 회귀 lock — §5.3 참조)
- **Phase B**: ✅ PR2 ~ ✅ PR5 **모두 완료** — Phase B 종료
- **Phase C**: ✅ PR6 (6.1 → **PR6.6 로 1.0 재진입** / 6.2~6.5 완료) / ✅ PR7 (Part 1 + Part 2 모두 완료) / ✅ PR8 (Part 1~3 **모두 완료**) / ✅ PR9 완료 — **재개 (PR6.6 진행 중)**
- **Phase D**: 🚧 PR10 Part 1 ✅ / PR10 Part 2 ✅ / **PR10 Part 3 ✅ (2026-05-30, SettingsPanel + CommandPalette 한국어 카피 통일)** / ✅ PR11 완료 (자동 측정) / ☐ PR12 미진입
- **추가**: ✅ PR-0c 완료 (clippy 48 errors → 0). `cargo clippy --all-targets -- -D warnings` 가 이제부터 진짜 lock.

진행 중 작업: **PR6.6 (file_snapshots — dogfood 발견 fix)**. fresh repo (HEAD-less) 에서 LocalDiffView 가 `fatal: bad revision 'HEAD'` 로 깨지는 회귀 발견 (2026-05-31). PR6.1 을 1.0 으로 당겨 hybrid (git + snapshot fallback) 구현. 남은 다른 PR 은 PR12 (빌드/서명/릴리스).

### 5.2 머지 로그

| 일자 | PR | 머지 커밋 | 비고 |
|---|---|---|---|
| 2026-05-31 | PR6.6 | `363d3e7` | `pre-cut-PR6.6` annotated tag (`15a5ac0`) 부여. dogfood 발견 회귀 fix — fresh repo (HEAD-less) 에서 LocalDiffView 가 `fatal: bad revision 'HEAD'` 로 깨지는 문제 해결. PR6.1 을 1.0 으로 당겨 hybrid `compute_diff` 구현. **scope 결정 변경 2건** (§5.3 참조): ① §0.6 잠금 `file_snapshots = per-path 50 LRU` → `per-path 단일 row` (1.1 에 LRU 50 재검토), ② 신규 cargo dep `similar 2.6` (MIT, ~70KB, unified-diff 생성) 도입 승인. zstd 는 미도입 (raw BLOB, 1.1 후보). **백엔드** (8 files, ~+450 LOC): ① 신규 마이그레이션 015 `file_snapshots(id, project_id, path, content BLOB, hash, captured_at, UNIQUE(project_id, path))` — 파일/버전 번호는 010 → 015 로 조정 (db.rs:21 의 기존 `(version 10, file 011_project_blueprints)` 충돌 회피, 버전 11 도 미사용이지만 안전하게 sequential 다음 15 채택), ② `Db::upsert_file_snapshot` / `Db::get_file_snapshot` + `FileSnapshot` struct (specta 제외 — 외부 노출 없음, `compute_diff` 가 내부 소비), ③ `commands::project::index_project` per-file 본문이 chunk insert 직후 `upsert_file_snapshot` 호출 (file 이 `changed=true` 일 때만, baseline = 인덱싱 직후 본문), ④ `commands::diff::reindex_paths` 도 동일 — 단 `changed=false` 라도 force-snapshot (caller 가 명시적으로 paths 지정 → "비우기 via reindex" 의미), ⑤ `compute_diff` hybrid 분기: `git::diff_patch` 성공 + non-empty → `DiffSource::Git`, 빈 patch OR `is_recoverable_git_failure(err)` (HEAD-less / non-git / unknown revision / ambiguous HEAD 4종) → `snapshot_diff` fallback, 그 외 err → bubble up. `DiffSource` 에 `Snapshot { patch }` variant 추가, `SnapshotsUnavailable` 은 진짜 snapshot 부재 시에만, ⑥ `snapshot_diff` 가 disk 본문 read → snapshot.content 와 동일 시 빈 patch, 아니면 `similar::TextDiff::from_lines` + `unified_diff().context_radius(3).header("a/<path>", "b/<path>")` → `diff --git a/<p> b/<p>` 헤더 prepend → max_bytes truncation, ⑦ 신규 커맨드 `resnapshot_paths(project_id, paths) -> u32` — disk 의 현 본문을 강제로 snapshot 으로 덮어쓰기 (blake3 재계산), LocalDiffView "비우기" 가 호출하는 baseline reset 의 backend, ⑧ `lib.rs` invoke_handler + `commands::*` re-export 에 `resnapshot_paths` 등록. **프론트엔드** (1 file, ~+50 LOC): ⑨ `LocalDiffView` 의 `onClear` 신설 — recentChanges paths dedup → `commands.resnapshotPaths` 호출 (실패 시 silent — best-effort baseline reset) → `clearRecentChanges` 호출. 기존 "비우기" 버튼이 `clearRecentChanges` → `onClear` 로 swap, tooltip "변경 목록 비우기 + 스냅샷 baseline 재설정", ⑩ `DiffBody` 가 `result.source.source === "snapshot"` 분기 추가 — 본문 위에 "로컬 스냅샷 기반 diff — 마지막 인덱스 이후 디스크 변경분" hint 박스 (muted 배경, rename 미감지 caveat), `SnapshotsUnavailable` 카피를 "1.1 fallback 안내" 에서 "부분 reindex 후 다시 보세요" 로 swap, 빈 patch 메시지에 source 별 baseline 명시 ("HEAD 와 동일" vs "스냅샷과 동일"), ⑪ `explainAvailable` + `onExplainAi` 가 snapshot 분기도 허용하도록 확장 (git 또는 snapshot 둘 다 OK, baseline 라벨 분기 "HEAD" vs "마지막 인덱스 시점"). **테스트**: ⑫ `tests/local_diff.rs` 신규 3 시나리오 — `diff_patch_signals_headless_repo_with_bad_revision_head` (git init only, no commit → `git::diff_patch` err 가 "bad revision 'HEAD'" or "ambiguous argument 'HEAD'" 포함 — upstream git stderr 변경 회귀 차단), `file_snapshot_upsert_is_idempotent_per_path` (Db round-trip: 첫 upsert insert + 두 번째 upsert update in place, id 보존으로 UNIQUE constraint 검증), `get_file_snapshot_returns_none_for_unindexed_path` (`DiffSource::SnapshotsUnavailable` 분기 driver), ⑬ `commands/diff.rs` inline `mod tests` 3개 (`recoverable_git_failures_cover_*` 6 assertions / `render_unified_diff_produces_git_compatible_headers` 5 assertions / `render_unified_diff_truncates_oversized_output` 2 assertions). vitest 변경 없음 (50 pass + 3 todo 유지 — `classifyDiffLines` 가 이미 unified-diff 표준 호환). cargo test **248 pass + 1 ignored** (was 242+1, +3 integration + +3 inline = +6). clippy clean (test mod 을 파일 끝으로 이동 — `items-after-test-module` 회피). typecheck/lint green. **연기 (1.1)**: zstd 압축 / LRU 50/path / watcher 자체 snapshot / 바이너리 파일 fallback ("(binary, no preview)") / `similar::Algorithm` Patience 등 옵션 튜닝. |
...
| 2026-05-29 | PR11 | `3286573` | local_diff integration suite + `_perf-1.0.md`. `pre-cut-PR11` annotated tag. 3 files (+184/-1, 신규 2 = `src-tauri/tests/local_diff.rs` + `docs/Lite-update/_perf-1.0.md`). ① 신규 통합 테스트 4 시나리오 — modified 파일 → unified diff +/- 라인 포함, unmodified 파일 → 빈 patch, 50k 라인 bloat 와 `max_bytes=4096` 으로 `"... (truncated, N bytes total)"` suffix + `cap+512B` budget 검증, non-git → `"Not a git repository."` sentinel (compute_diff 의 `SnapshotsUnavailable` fallback 가 이 문자열로 분기). 모든 assertion < 1s, helper 가 `GIT_CONFIG_GLOBAL=/dev/null` 셋업으로 호스트 .gitconfig 오염 차단. ② `mod git` → `pub mod git` 승격 (PR8 Part 1 의 `pub mod oculpm` 패턴 답습) — 통합 suite 가 `git::diff_patch` 를 Tauri State 없이 직접 호출 가능. ③ `docs/Lite-update/_perf-1.0.md` — §1 자동 측정 (50k bench 112ms / SLO-A1~A5 모두 통과 + 재현 명령), §2 dogfood SLO (idle CPU/mem / ndjson append p95 / 100-file batch / 마이그레이션 100 entries / Today 카드 로드 — Overview→Today 재정의), §3 PR12 직전 회귀 체크리스트, §4 1.1 이월 (embedder warm-up bench / Terminal 버벅거림 root cause / react-virtual 100k 대응). cargo test **242 pass** (220 lib + 7 agents + **4 local_diff 신규** + 5 lite_w6 + 6 oculpm_migration) + 1 ignored. clippy clean. vitest / typecheck / lint 변경 없음. |
| 2026-05-30 | PR6.5 | `cf00374` | LocalDiffView UX polish — 4 sub-feature 일괄 완료. `pre-cut-PR6.5` annotated tag. 6 files (+446/-37). ① **SidePanel diff mode 폭 1100px** — `effectiveSidePanelMaxWidth(mode)` helper (`files=500`/`diff=1100`), `migrateSidePanelWidth` 의 max 를 1100 으로 확장 (사용자가 diff 에서 늘린 폭이 files 로 잠시 다녀와도 정보 손실 X). SidePanel 의 drag clamp + render `style.width` 가 mode-aware. ② **ResizeObserver 기반 side-by-side ≥1024px** — `containerRef` 가 LocalDiffView root 의 painted width 를 추적, `SIDE_BY_SIDE_BREAKPOINT_PX=1024` 이상이면 `SideBySideHunk` 렌더 (CSS grid 2-col). `pairDiffLines` pure-fn: 연속 deletion 블록 + 직후 연속 addition 블록을 index 별 zip, longer 쪽은 opposite=null 로 padding, context/header/hunk 는 양쪽 mirror. ③ **Collapse long hunks** — `groupIntoHunks` pure-fn 이 `@@` 마커마다 hunk 분리 + file-header preamble 도 별도 hunk, `HUNK_FOLD_THRESHOLD=20` 이상은 default-fold. fold 상태에서 헤더 텍스트 + "N줄 더 보기" 토글, expand 후 "접기" 버튼으로 재축소. unified / side-by-side 양쪽 동일 동작. ④ **읽음/안읽음** — `RecentChange` 에 `read: boolean` field 추가. watcher 의 신규 이벤트는 `read: false` 로 push, `loadFromStorage` 가 legacy 엔트리 (PR6.5 이전) 를 `true` 로 마이그레이션해 upgrade 시 폭증 회피. `markRecentChangeRead(path)` action 신설 (path 미존재 / 이미 read 면 no-op setState churn 방지). LocalDiffView 의 useEffect 가 selected + !loading + !error + diff 모두 충족 시 호출 (diff body 가 실제 painted 후에만 read). FileRow 가 `unread ? font-semibold + primary dot : muted dot`, 헤더에 `안읽음 N` counter. ⑤ **AI 에게 설명** — LocalDiffView 헤더 우상단 `Sparkles` 버튼 (`explainAvailable` 일 때만 active = git source + 비-에러 + 비-loading + 패치 존재). 클릭 시 prompt = `"다음 diff 변경에 대해 설명해 주세요. (파일: <path>)\n\n```diff\n<patch>\n```"` (또는 patch 가 비면 "HEAD 와 동일한 상태" 라인) 을 만들어 `window.dispatchEvent(new CustomEvent("ai-overlay:prefill", { detail: { prompt } }))`. ChatPanel.tsx 에 listener 추가 — prefill 받으면 setInput; 사용자가 이미 input 에 텍스트 있으면 prefill 을 앞에 prepend (사용자 작업 보존). 동시에 `setAiOverlayOpen(true)` 로 ⌘\ 와 동일한 효과. ⑥ 회귀: `groupIntoHunks` (2-marker patch → 3 hunks, 첫 preamble header=null), `pairDiffLines` (deletion/addition zip / unpaired addition right-only / context mirror), `effectiveSidePanelMaxWidth` (files=500 / diff=1100). 기존 RecentChange 테스트 5건 + clearRecentChanges 1건이 `read: false` 추가됨. `migrateSidePanelWidth` clamp 테스트 갱신 (`SIDE_PANEL_MAX_WIDTH_DIFF` 기준). vitest **50 pass + 3 todo** (was 46+3). cargo test 242 + 1 ignored (Rust 변경 없음). clippy clean. typecheck/lint green. 신규 dep 0. |
| 2026-05-30 | PR10 (Part 3) | `e04b5ec` | SettingsPanel + CommandPalette 한국어 카피 통일. `pre-cut-PR10-part3` annotated tag. 3 files (+109/-109, code 변경 0 줄 net — pure copy swap). ① `SettingsPanel.tsx` 의 TABS labels (`Appearance/Indexing & RAG/Graph/Data/Diagnostics → 모양/인덱싱 & RAG/그래프/데이터/진단`) + 8 탭 Section/Field/hint/button — 약 50 문자열 한국어화. AppearanceTab: 외부 에디터 / 테마 / 코드 에디터 (PR5 legacy 이동 inline comment 추가 — 1.1 까지 보존). LlmTab: API 키 / 기본 프로바이더 / 모델 / 생성 + Temperature 보존 + 시스템 프롬프트 placeholder ("당신은 도움이 되는 코딩 어시스턴트입니다…"). GithubTab: 개인 액세스 토큰 (병기) / 연결된 계정 / 로컬 git. IndexingTab: 청킹 / 검색 (Retrieval 병기) / 파일 스캔. GraphTab: 의존성 그래프 기본값. DataTab: 저장소 / 진단 / 초기화 / 위험 구역 + 라벨 (Database/App data/Secrets/Version → 데이터베이스/앱 데이터/비밀 데이터/버전). DiagnosticsTab: 데이터베이스 상태 + "조회되지 않음" fallback. ② `CommandPalette.tsx` — `aria-label="Command Palette" → "명령 팔레트"`, view-* labels (Today/Plan/Code → "오늘 (Today)" 등 병기), code-* labels (Files/AI/Graph/Terminal → 파일/AI/그래프/터미널 병기), group `"Code 화면" → "코드 화면"` (type union + render 모두), "Settings 열기" → "설정 열기", "Overview 다시 생성" → "개요 다시 생성". alias 에 영문 원본 보존 — 사용자가 영어로 검색해도 매칭. ③ ko.json 변경 X — i18n 시스템이 wire 되어 있지 않음 확인 (`grep useTranslation\|i18n\|t\(\"` 결과 0). stale legacy 로 명시, 1.1 에 i18n hook 도입 시 sweep. **기술 명사 보존**: API / LLM / RAG / GitHub / Anthropic / OpenAI / Gemini / NVIDIA / SQLite / sqlite-vec / Temperature / Personal Access Token (병기) / JSON / KB / gitignore / D2Coding. **단축키 보존**: ⌘1·2·3·B·\\·,·J·ESC 등 모두 영문. typecheck/lint/vitest 49 pass + 3 todo (변경 없음). cargo + clippy 무관 (Rust 미변경). **연기 (1.1 후보)**: 잔존 영문 카피 sweep — Today/Plan/Code 메인 컨텐트, Modals (StartScreen/MigrationModal/OculpmOnboardingModal/LegacyDeleteModal/AiOverlay), SidePanel/TerminalDock/TitleBar/GitBranchChip. ko.json 의 i18n wire (react-i18next 또는 자체 hook). |
| 2026-05-30 | PR10 (Part 2) | `c267b93` | axe-core a11y 풀-스윕 — Today/Plan/Settings 3 화면. `pre-cut-PR10-part2` annotated tag. 7 files (+160/-7, 신규 1 = `src/__tests__/a11y_screens.test.tsx` 98 lines). ① `package.json` devDeps 에 `vitest-axe@0.1.0` (MIT, jest-axe fork by Chance Strickland) + `axe-core@4.11.4` (MPL-2.0, Deque Systems). 둘 다 dev-only — 번들 영향 0. ② NEW `src/__tests__/a11y_screens.test.tsx` — `vi.mock("@/lib/bindings", ...)` 가 Proxy 로 모든 command 를 `{status:"ok", data:<empty default>}` stub (listProjects/goalList/subtaskList/settingsGetAll/dbHealth/appInfo/secretHas 분기 + 기본 null), `vi.mock("@/api/oculpm", ...)` 도 동일 패턴. 3 describe — Today/Plan/Settings 화면을 `<SettingsProvider><WorkspaceProvider>...` 안에 render 후 `axe(container, AXE_OPTIONS)` 호출 + `summarize(r.violations)` 가 `{id, help, nodes}` 배열을 `[]` 와 비교 (vitest-axe@0.1.0 의 `toHaveNoViolations` matcher 는 vitest 4 의 namespace `Vi` 가 더 이상 augment 가능하지 않아 type 인식 실패 — matcher 우회). `color-contrast` 와 `region` rule 은 jsdom 한계로 disable (CI/Playwright 이월). ③ `src/__tests__/setup.ts` 에 `window.matchMedia` polyfill 추가 — `SettingsContext` 가 theme effect 에서 호출 → jsdom 부재로 throw → mount 자체 실패 회귀 차단. 기본 `{matches: false}` 로 light theme 가정. ④ axe 가 PlannerPanel 의 두 Select trigger (`상태 필터 .w-[120px]` / `프로젝트 필터 .w-[140px]`) 에서 `button-name` 위반 (Radix Select 의 SelectValue placeholder 만 있을 때 inner text 부재) 적발 — `SelectTrigger` 에 `aria-label="상태 필터" / "프로젝트 필터"` 추가. 사용자가 close 된 trigger 를 screen reader 로 들었을 때 어떤 필터인지 즉시 인식 가능. ⑤ `scripts/check-no-localstorage.mjs` ALLOWLIST 에 a11y_screens.test.tsx 추가 — 각 화면 mount 전 `localStorage.clear()` 가 WorkspaceContext default state 재사용을 위한 test-only seeding. **연기**: 모달 (StartScreen / MigrationModal / OculpmOnboardingModal / LegacyDeleteModal / AiOverlay / CommandPalette) + 사이드/도크 (SidePanel / FileExplorer / TerminalDock / TitleBar / GitBranchChip) + Settings 내부 8 탭 의 풀-스윕 a11y 는 1.0 dogfood 후 1.1 후보. color-contrast 자동 측정은 Playwright/CI 영역. vitest **49** pass (was 46) + 3 todo. cargo test 242 + 1 ignored. clippy clean. typecheck/lint green. |
...
| 2026-05-31 | **dogfood 회귀** — 초기 커밋이 없는 fresh repo 에서 LocalDiffView 가 `fatal: bad revision 'HEAD'` 로 깨짐. watcher 는 변경을 정상 감지 ("변경된 파일 2개") 했으나 backend `git::diff_patch` 가 `HEAD` 미존재로 raw stderr 반환. UX 상 "변경이 있는데 없다고 뜸". | PR6.1 을 **1.0 으로 당김 → PR6.6** (Phase C 재개). hybrid `compute_diff`: git path 우선 + HEAD-less/non-git 시 snapshot fallback. **결정 변경 1**: §0.6 의 `file_snapshots 보관 = per-path 최근 50 LRU` → **per-path 단일 row (1.1 에 LRU 50 결정 재검토)**. **결정 변경 2**: master-prompt §6 rule 6 에 의거, 신규 cargo dep `similar 2.x` (MIT, ~70KB, unified-diff 생성) 도입 승인 (사용자 2026-05-31 확인). zstd 는 **미도입** — raw BLOB 저장, 1.1 에 압축 추가 후보. | §5.1 (PR6.6 재개), 07-checklist §0.6 (잠금 갱신), 05-index-comparison.md §4.2 (zstd → 1.1) |
