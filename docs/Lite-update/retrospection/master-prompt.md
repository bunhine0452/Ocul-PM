...
- **Phase C**: 🚧 PR6 (backend 만, 6.1 연기 / 6.2 완료 / 6.3~6.5 미진입) / ✅ PR7 (Part 1 + Part 2 모두 완료) / ☐ PR8~PR9 미진입
- **Phase D**: ☐ PR10~PR12 미진입
- **추가**: ✅ PR-0c 완료 (clippy 48 errors → 0). `cargo clippy --all-targets -- -D warnings` 가 이제부터 진짜 lock.

진행 중 작업: *없음*. 다음 작업: **PR8 (FileTree 재설계 + ⌘B 사이드 패널)** — Phase C 의 마지막 큰 frontend. PR7 의 layoutMode 위에 ⌘B 좌측 사이드 패널 추가. PR9 (AI overlay) 와 PR6.3 (LocalDiffView UI) 도 PR8 이후.
