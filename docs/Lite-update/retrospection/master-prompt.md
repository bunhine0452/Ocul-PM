...
- **Phase C**: 🚧 PR6 (backend 만, 6.1 연기 / 6.2 완료 / 6.3~6.5 미진입) / 🚧 PR7 (Part 1 완료 — IA collapse + GitBranchChip / Part 2 = layoutMode + TerminalDock 미진입) / ☐ PR8~PR9 미진입
- **Phase D**: ☐ PR10~PR12 미진입
- **추가**: ✅ PR-0c 완료 (clippy 48 errors → 0). `cargo clippy --all-targets -- -D warnings` 가 이제부터 진짜 lock.

진행 중 작업: *없음*. 다음 작업: **PR7 Part 2 (layoutMode + TerminalDock)** 또는 **PR8 (FileTree 재설계)** 또는 PR6.3 (LocalDiffView UI). PR8 은 PR7 Part 2 (⌘B 사이드 패널) 의존이 있으므로 자연 순서는 PR7 Part 2 → PR8 → PR9.
