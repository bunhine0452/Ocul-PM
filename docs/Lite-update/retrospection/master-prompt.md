...
- **Phase C**: 🚧 PR6 (backend 만, 6.1 연기 / 6.2 완료 / 6.3~6.5 미진입) / ✅ PR7 (Part 1 + Part 2 모두 완료) / 🚧 PR8 (Part 1 + Part 2 완료 / Part 3 미진입) / ☐ PR9 미진입
- **Phase D**: ☐ PR10~PR12 미진입
- **추가**: ✅ PR-0c 완료 (clippy 48 errors → 0). `cargo clippy --all-targets -- -D warnings` 가 이제부터 진짜 lock.

진행 중 작업: *없음*. 다음 작업: **PR8 Part 3 (a11y ↑↓←→ + "변경 표시 비우기" 액션 + 50k 파일 마운트 < 500ms 측정)**. PR9 (AI overlay) 와 PR6.3 (LocalDiffView UI) 은 PR8 종료 후.
