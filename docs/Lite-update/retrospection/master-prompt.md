...
- **Phase C**: 🚧 PR6 (6.1 1.1 로 연기 / 6.2 완료 / 6.3 완료 / 6.4~6.5 미진입) / ✅ PR7 (Part 1 + Part 2 모두 완료) / ✅ PR8 (Part 1~3 **모두 완료**) / ✅ PR9 완료
- **Phase D**: ☐ PR10~PR12 미진입
- **추가**: ✅ PR-0c 완료 (clippy 48 errors → 0). `cargo clippy --all-targets -- -D warnings` 가 이제부터 진짜 lock.

진행 중 작업: *없음*. **Phase C 사실상 종료** — PR6.4~6.5 의 마무리만 남음 (FileTree dot click 핸드오프 / Today 카드 진입 / side-by-side / 읽음 표시 / "AI 에게 설명"). 다음 후보: **PR6.4~6.5 마무리** 또는 **Phase D PR10 (a11y 풀-스윕 + 다크모드 + 카피 통일) 진입**. 1.0 출시까지 최단 경로는 Phase D 직진.
