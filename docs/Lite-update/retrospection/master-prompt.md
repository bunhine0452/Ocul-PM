...
- **Phase C**: 🚧 PR6 (6.1 1.1 로 연기 / 6.2 완료 / 6.3 완료 / 6.4~6.5 미진입) / ✅ PR7 (Part 1 + Part 2 모두 완료) / ✅ PR8 (Part 1~3 **모두 완료**) / ☐ PR9 미진입
- **Phase D**: ☐ PR10~PR12 미진입
- **추가**: ✅ PR-0c 완료 (clippy 48 errors → 0). `cargo clippy --all-targets -- -D warnings` 가 이제부터 진짜 lock.

진행 중 작업: *없음*. 다음 작업: **PR9 (AI overlay ⌘\ + 분리 윈도우 ⌘⇧\)** — 사용자가 PR6.3 → PR9 순서 선택. PR6.4~6.5 는 PR9 종료 후 진입 가능 (FileTree dot click 핸드오프 / side-by-side / 읽음 표시 / "AI 에게 설명").
