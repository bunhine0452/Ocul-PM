...
| GR1 | 멀티관계 스키마 + 빌더 + `get_code_graph` (imports/contains) | GR0 | ☐ | — |
...
## PR-GR1 — 멀티관계 스키마 + 빌더 (imports/contains)

**목표:** SQLite 멀티관계 골격. calls 추출 전, 무비용 엣지(contains)부터.

- ☐ `migrations/006_code_graph.sql` — `graph_nodes`/`graph_edges` ([`01`](./01-data-model-and-schema.md) §2)
- ☐ `src-tauri/src/graph.rs` — `rebuild_project()` (file/symbol 노드 upsert, contains, imports 백필) — **LLM-free**
- ☐ 인덱싱 끝에 `rebuild_project` 훅(증분: changed_file_ids)
- ☐ `commands/graph.rs` — `get_code_graph(project_id, opts)` + DTO
- ☐ `get_dependency_graph` 를 `get_code_graph` 투영으로 유지(하위호환, D-E) — 출력 동일 회귀 테스트
- ☐ GR0 화면을 `getCodeGraph` 로 전환(심볼펼침 토글 자리만, calls 는 GR2)

**DoD:** `graph_edges` 에 imports+contains 채워짐. 심볼 펼침 토글 시 파일→심볼 contains 보임. 결정론 테스트 ✓. 하위호환 회귀 ✓. `cargo test` ✓.
