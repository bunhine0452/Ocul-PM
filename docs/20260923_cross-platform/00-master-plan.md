...
| 결정 | 막는 곳 | 선택지 |
|---|---|---|
| Windows 코드 서명 | W4 | **결정됨 (2026-09-24): 무서명 베타로 시작** — SmartScreen "추가 정보 → 실행" 안내, 서명은 나중에 붙일 수 있게 |
| Linux 형식 확정 | W3 | **결정됨 (2026-09-24): AppImage + deb** (D10 그대로, rpm 은 나중에) |
| Windows 업데이트 때 터미널 세션 | W3 | **결정됨 (2026-09-24): 살린다** — 호스트를 설치 폴더 밖 판별 복사본에서(L-PTY2) |
| macOS 딸린 claude 진단 결함 | — | **결정됨 (2026-09-24): 고친다** — 진단만 바뀐다(실제 실행되는 claude 는 어댑터가 고름), PR #39 |
| Windows VC++ 재배포 의존 (출시 차단) | W3·W4 | **결정됨 (2026-09-25): 설치 파일이 없을 때만 자동 설치** — System32 에 깔려 호스트 복사본·심까지 풀린다, 없는 PC 에서만 UAC 한 번, 설치 파일 +25MB |
| Windows 최소 버전 (DirectML `DMLCreateDevice1`) | W3·W4 | **결정됨 (2026-09-25): Windows 10 2004(19041) 미만은 설치 파일이 막고 안내** |
| W4 진행 | W4 | **결정됨 (2026-09-25): 준비까지 진행** — 실제 태그·공개 릴리스·랜딩 배포·버전 올리기는 따로 묻는다 |
