...
## 7. 구현 기록 (2026-09-01 · v2.28.0)

7항목 전부 구현. 설계와 **다르게 간 것 여섯 가지**를 여기 남긴다 — 설계 문서만
읽고 코드를 의심하지 않게.

### 7.1 화이트리스트는 편집기 다섯 그룹과 **같다** (31개)

§1 은 "`tokens.css` 에서 추출한 목록" 이라고만 적었다. 실제로는 §4 의 다섯 섹션
(배경 7 · 글자 4 · 강조 5 · 경계 3 · 상태색 12)으로 정확히 **닫았다**. 트리거
색(`--t-*`)·diff·터미널 ANSI 는 뺐다.

근거: 편집기에 노출하지 않는 토큰을 임포트로만 칠할 수 있게 하면 **되돌릴 방법이
없는 색**이 생긴다 (「가족 기본값으로 되돌리기」가 그 토큰 행을 안 그리므로).
의미색은 가족에서 상속되는 편이 옳기도 하다. 내장 프리셋이 쓰는 19개는 전부 이
안에 있다.

목록은 두 언어에 있다 — 검증은 신뢰 경계(백엔드)에 있어야 하고, 편집기는 그룹과
라벨이 필요하다. `src/__tests__/theme_schema.test.ts` 가 `themes/mod.rs` 의
`ALLOWED_TOKENS` 를 파싱해 프런트 `TOKEN_GROUPS` 와 대조한다.

### 7.2 내장 5종은 **프런트에만** 산다

§1 은 "빌드 타임에 JSON 생성" 이라 적었고, 그 산출물을
`src/features/theme/builtin/*.json` 에 체크인한다 (`scripts/gen-builtin-themes.mjs`).
백엔드 `theme_list` 는 **사용자 테마만** 돌려준다 — 내장은 파일이 아니라 코드이고,
`tokens.css` 가 프런트 자산이라 백엔드를 지날 이유가 없다. 디스크에서 읽은 테마는
`is_built_in` 을 항상 `false` 로 강제한다.

### 7.3 프로젝트 바인딩 값은 설정 `theme` 와 **같은 축**

`034` 의 첫 초안은 `builtin:<name>` 접두를 생각했지만, 저장하는 값을 설정의
`theme` 와 같은 문자열(`"dark"` · `"solarized"` · `"custom:<uuid>"`)로 두었다.
D3 과 같은 이유다: 축이 둘이면 매핑 표가 생기고, 그 표가 부채가 된다. 그래서
"프로젝트 바인딩" 은 **그 프로젝트에서 이기는 `theme` 설정값**이라는 한 문장으로
설명된다.

창 단위 적용은 **활성 탭**이 정한다 — 창 하나가 탭 여럿을 물고 `<html>` 은 창에
하나뿐이므로 (`TabbedWindow`). 떼어낸 터미널 창도 자기 프로젝트의 바인딩을 쓴다.

### 7.4 대비 테스트는 "유지" 가 아니라 **신설**

§6 은 "a11y 대비비 회귀 테스트 유지" 라고 적었으나 그런 테스트는 없었다 —
`a11y_screens.test.tsx` 는 axe 의 `color-contrast` 를 **끈다** (jsdom 에 레이아웃이
없어 계산이 불가능하다). 대신 토큰 값에서 직접 WCAG 대비를 계산하는 테스트를
만들었다: 내장 5종 전부 본문 4.5:1 이상, `high-contrast` 는 7:1 이상.
레이아웃이 필요 없는 순수 산술이라 jsdom 에서 성립한다.

### 7.5 `useTheme()` 은 이제 DOM 을 읽는다

`src/lib/theme.tsx` 는 설정값에서 가족을 **다시 유도**하고 있었다. 가족을 정하는
입력이 넷(전역 설정 · 프로젝트 바인딩 · 편집 초안 · 테마 파일)으로 늘어난 지금
두 벌로 계산하면 반드시 어긋난다 (다크 커스텀 테마에서 코드 하이라이트만 라이트로
그려지는 식으로). `SettingsContext` 가 이미 `<html data-theme>` 에 답을 썼으므로
그것을 `MutationObserver` 로 읽는다. 소유자는 하나다.

같은 이유로 `PRESET_FAMILY` 는 손으로 적은 표를 버리고 내장 테마 파일에서
유도한다.

### 7.6 모바일은 커스텀 테마를 해석하지 못한다

`src/mobile/theme.ts` 는 맥의 설정을 읽어 폰에 같은 색을 입힌다. 커스텀 테마의
토큰은 맥의 앱 데이터에 있는 **파일**이고 모바일 커맨드 화이트리스트 밖이라,
`custom:` 값을 만나면 OS 다크모드로 떨어진다. 임의로 라이트를 고르지 않는다.

### 남긴 것

- 임포트 사본 이름은 `"{name} (2)"` 고정이다 — 세 번째 사본도 `(2) (2)` 가 된다.
  드문 경로라 번호 찾기를 넣지 않았다.
- 시스템 강조색은 창 포커스마다 다시 읽는다. macOS 의 분산 알림을 듣지 않으므로
  앱이 포커스를 잃은 동안 바뀐 색은 다음 포커스에 반영된다.
