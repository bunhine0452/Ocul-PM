...
> **전제 정정 (2026-09-04)** — 결정 자체(오버레이를 복원하지 않는다)는 그대로다.
> 다만 "`AiPanelScreenV2` 가 유일한 채팅 표면" 은 **이 문서를 쓸 때 이미
> 사실이 아니었다**: ACP 패널(v2.10.0)이 이미 `ClaudeCodeScreenV2` 를
> 들여왔고, 그 뒤 `CodexScreenV2` 가 합류해 채팅 표면은 **셋**이다
> (`src/features/chat/`).
> 은퇴한 것은 **오버레이**이지 "패널 하나" 가 아니다.
