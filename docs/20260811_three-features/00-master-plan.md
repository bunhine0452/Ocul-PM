...
| # | 기능 | 결정 |
|---|---|---|
| 1 | 멀티 프로젝트 창 | **메인 창 = 런처 전용**. 프로젝트를 열면 항상 별도 창(`project-<id>`). 메인은 다시는 셸을 띄우지 않는다 — **뒤집힘, 아래 참고** |
...
> **1번 결정은 뒤집혔다 (v2.9.0 크롬식 탭)** — "런처 전용 창" 은 **시작 탭**이
> 대체했다 (`windows/StartTab.tsx`). 한 창이 탭 여러 개를 물고, 탭마다
> `WorkspaceProvider` + `ProjectTab` 이 셸을 마운트한다 (`windows/TabbedWindow.tsx`).
> 창 라벨도 `project-<id>` 고정이 아니다 — 탭이 창 사이를 오간다.
> `main.tsx` 의 갈림길 주석이 같은 말을 한다.
