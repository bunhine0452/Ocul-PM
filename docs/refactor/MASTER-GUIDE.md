...
>
> 2026-05-20 의 계획이다. 아래의 "향후 모든 PR·이슈·논의는 이 문서를 기준으로" 는
> **더 이상 유효하지 않다** — 현재 설계의 진입점은
> [`../README.md`](../README.md) 이고, 진행은 `.oculpm/planner/` 다.
> 아래 두 문장은 특히 현재 코드와 정면으로 어긋난다:
> ① **"코드를 직접 수정하지 않고"** — 앱은 이제 코드를 쓴다 (`code_write`,
>    LSP 이름 바꾸기, DAP 디버거, `commands/code.rs` 의 생성·삭제·이름변경).
> ② **5화면 IA** (§5.1) — 실제 화면은 **16개**다 (`src/lib/navRegistry.ts`).
...
> **실현되지 않았다 (2026-09-04 확인)** — 사이드바는 5개로 줄지 않았고 지금
> **16개**다 (`src/lib/navRegistry.ts` — 앞 10개만 ⌘1~⌘0 을 갖는다). 아래 표는
> 2026-05-20 의 제안으로만 읽을 것.
