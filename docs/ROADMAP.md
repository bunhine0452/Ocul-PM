...
#### M3-4. RAG 질의 ✅ (Phase A)
- [x] **검색 단독 동작**: 자연어 → 임베딩 → top-K 청크 (`searchChunks` + UI)
- [x] **채팅 통합**: 컨텍스트 프로젝트 선택 → 매 사용자 메시지마다 top-K 청크를 시스템 프롬프트로 자동 주입
- [x] 사용된 청크를 사용자 메시지 아래 collapsible 배지로 표시 (citation)
- [x] **Reindex** 버튼 — 전체 청크 삭제 후 재인덱싱
- [x] lockfile/sourcemap/min.js 자동 제외 필터
