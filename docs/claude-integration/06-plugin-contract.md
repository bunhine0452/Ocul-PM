...
| 훅 세션 마커 (SessionStart 3번째 — `hooks/session-marker.sh`) | 세션 시작 | stdin 의 session_id | `.oculpm/hooks/.session-start-<sid>` 마커(빈 파일) — 7일+ 잔여는 자체 청소 | SessionEnd 의 "이 세션에서 일지가 났는가" 판정 기준점 외 다른 용도 없음 |
